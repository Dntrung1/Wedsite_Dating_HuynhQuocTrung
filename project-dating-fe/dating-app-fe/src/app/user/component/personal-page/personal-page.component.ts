...
export class PersonalPageComponent implements OnInit {
  ...
  constructor(private userService: UserServiceService,
              ...
              private toastrService:ToastrService,private toast: ToastrService) {
  ...
  show(event: any) {
    ...
    for (let i = 0; i < this.links.length; i++) {
        this.linkConcact.push(this.links[i])
    }
  ...
  async handleFiles() {
    for (let i = 0; i < this.linkConcact.length; i++) {
      await this.uploadFile(this.linkConcact[i])
    }
  }
  ...
  savePost() {
    ...
    this.handleFiles().then(() => {
      ...
      this.postService.create(this.postCreate.value).subscribe(() => {
        ...
        // this.toast.warning('Đăng bài thất bại', "Thông báo")
      });
    }).finally(() => {
      this.postCreate.reset();
      window.location.reload();
      this.ngxService.stop();
      this.toast.success('Đăng bài thành công', "Thông báo")
    })
  }
